...
type Props = DrawerScreenProps<DrawerParamList, 'EditaRefeicao'>;

const EditaRefeicaoScreen = ({ route, navigation }: Props) => {
  ...
  const [horario, setHorario] = useState(refeicao.horario.substring(0, 5));
  const [descricao, setDescricao] = useState(refeicao.descricao || '');
  const [planoId, setPlanoId] = useState(refeicao.plano_alimentar?.toString() || '');
  
  const [alimentosSelecionados, setAlimentosSelecionados] = useState<number[]>(refeicao.alimentos || []);
  ...
  useEffect(() => {
    ...
    setHorario(refeicao.horario.substring(0, 5));
    setDescricao(refeicao.descricao || '');
    setPlanoId(refeicao.plano_alimentar?.toString() || '');
    setAlimentosSelecionados(refeicao.alimentos || []);
  }, [refeicao]);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/refeicoes/${refeicao.id}/`, {
        method: 'PUT',
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <TextInput placeholder="Horário" value={horario} onChangeText={setHorario} style={styles.input} maxLength={5} />
      ...
      <Text style={styles.label}>Selecione os Alimentos</Text>
...
export default EditaRefeicaoScreen;