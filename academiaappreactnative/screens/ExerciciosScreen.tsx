...
import { DrawerParamList, Exercicio } from '../navigation/types';

type Props = DrawerScreenProps<DrawerParamList, 'Exercicios'>;

const ExerciciosScreen = ({ navigation }: Props) => {
  ...
  const handleDelete = async (id: number) => {
    await fetch(`http://localhost:8000/exercicios/${id}/`, {
      method: 'DELETE',
    });
  ...
  const renderItem = ({ item }: { item: Exercicio }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.description}>Dificuldade: {item.dificuldade}</Text>
      
      <View style={styles.row}>
        <TouchableOpacity
          style={styles.editButton}
          onPress={() => navigation.navigate('EditaExercicio', { exercicio: item })}
        >
  ...
  return ( 
    <View style={styles.container}>
      ...
      <TouchableOpacity
        style={styles.fab}
        onPress={() => navigation.navigate('CriaExercicio')}
      >
