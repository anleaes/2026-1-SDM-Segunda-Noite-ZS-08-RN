...
type Props = DrawerScreenProps<DrawerParamList, 'EditaPlanoAlimentar'>;

const EditaPlanoAlimentarScreen = ({ route, navigation }: Props) => {
  const { plano } = route.params;

  const [titulo, setTitulo] = useState(plano.titulo);
  const [objetivo, setObjetivo] = useState(plano.objetivo);
  const [calorias, setCalorias] = useState(plano.calorias_diarias.toString());
  const [dataInicio, setDataInicio] = useState(plano.data_inicio);
  const [dataFim, setDataFim] = useState(plano.data_fim || '');
  const [descricao, setDescricao] = useState(plano.descricao || '');
  const [alunoId, setAlunoId] = useState<number>(plano.aluno);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await fetch(`http://localhost:8000/planosalimentares/${plano.id}/`, {
        method: 'PUT',
...
export default EditaPlanoAlimentarScreen;